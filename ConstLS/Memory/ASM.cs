 ...
 * Later (27.09.2012) it was modified by krukovis (http://zhyk.ru/forum/showthread.php?t=521124).
 */
...
namespace ConstLS.Memory
{
    class ASM
    {
        ...
        private string hexUint(uint address)
        {
        ...
        public string uintTohex(uint value, int num)
        {
            string str1 = null;
            string str2 = "";
            str1 = "0000000" + this.hexUint(value);
        ...
        {
            if ((addre <= 127) && (addre >= -128)) {
                this.Asmcode = this.Asmcode + "8B79" + intTohex(addre, 2);
            }
            else {
        ...
        {
            if ((addre <= 127) && (addre >= -128)) {
                this.Asmcode = this.Asmcode + "8B72" + intTohex(addre, 2);
            }
            else {
                this.Asmcode = this.Asmcode + "8BB2" + intTohex(addre, 8);
            }
        }
        ...
        {
            this.Asmcode = this.Asmcode + "68" + uintTohex(addre, 8);
        ...
        public byte[] inBytes()
        {
            byte[] reAsmCode = new byte[this.Asmcode.Length / 2];

            for (int i = 0; i <= reAsmCode.Length - 1; i++) {
                reAsmCode[i] = Convert.ToByte(Int32.Parse(this.Asmcode.Substring(i * 2, 2), System.Globalization.NumberStyles.AllowHexSpecifier));
            }
